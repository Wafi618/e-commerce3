...
import { Package, DollarSign, TrendingUp, CreditCard, Plus, Edit2, Trash2, MessageSquare, Key, Eye, EyeOff, BarChart2, Bell, Layout, GripVertical } from 'lucide-react';
...
// ... (existing helper function: reorder)
...
export default function AdminPage() {
  ...
  const {
    ...
    showProductModal,
    saveProduct,
    fetchProducts // Ensure this is available in context or just rely on prop updates if context handles it
  } = useProduct();
  ...
  const onDragEnd = async (result: any) => {
    ...
    try {
      // Create payload with ALL items and their new index as sortOrder
      const payload = {
      ...
      // Refetch from context to ensure global state is synced with DB
      fetchProducts();
    } catch (err) {
      console.error('Failed to reorder', err);
      // Revert on error? 
      setLocalProducts(products);
      addNotification('Failed to reorder products', 'error');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-900">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex gap-4 mb-8 overflow-x-auto">
          {['products', 'Gemini Agent', 'ai-chat', 'orders', 'overview', 'messages', 'customers', 'analytics', 'announcements', 'landing', 'settings'].map((tab) => (
            <button
              ...
              className={`px-4 py-2 rounded-lg whitespace-nowrap ${adminTab === tab ? 'bg-blue-600 text-white' : 'bg-gray-800 text-gray-300 hover:bg-gray-700'
                } capitalize`}
            >
              {tab.replace('-', ' ')}
            </button>
              ...
              <DragDropContext onDragEnd={onDragEnd}>
                <table className="w-full">
                  ...
                  <Droppable droppableId="products-list">
                    {(provided) => (
                      <tbody className="divide-y divide-gray-700" {...provided.droppableProps} ref={provided.innerRef}>
                          ...
                          <Draggable key={product.id.toString()} draggableId={product.id.toString()} index={index}>
                            {(provided, snapshot) => (
                              <tr
                                ...
                                className={`hover:bg-gray-700 ${(product as any).isArchived ? 'opacity-50' : ''} ${snapshot.isDragging ? 'bg-gray-700 shadow-lg' : ''}`}
                              >
                                ...
                                <td className="px-6 py-4 whitespace-nowrap">
                                  <div className="flex items-center gap-3">
                                    <div className="w-10 h-10 bg-gray-700 rounded flex items-center justify-center overflow-hidden flex-shrink-0">
                                      <img
                                        src={getImageUrl((product as any).image || (product as any).options?.find((o: any) => o.values.find((v: any) => v.image))?.values.find((v: any) => v.image)?.image || '') || '/placeholder.svg'}
                                        alt={product.name}
                                        className="w-full h-full object-cover transition-transform duration-300"
                                        style={{ transform: `rotate(${(product as any).imageRotation || (product as any).options?.find((o: any) => o.values.find((v: any) => v.image))?.values.find((v: any) => v.image)?.rotation || 0}deg)` }}
                                        onError={(e) => {
        ...
        {adminTab === 'Gemini Agent' && <AgentTab />}
